...
import {EditSalesRequest} from '../../../redux/userActions';
import {useDispatch} from 'react-redux';
import auth from '@react-native-firebase/auth';

const EditSaleProperty2 = (props: any) => {
  const {navigation} = props;
  const user = auth().currentUser;

  const dispatch = useDispatch();

  const saleData = props.route.params.data;
  ...
  const onPublish = async () => {
    ...
    setTimeout(() => {
      // console.log('formValues1', props.route.params.formValue1);
      // console.log('formValues2', props.route.params.formValue2);
      ...
      dispatch(EditSalesRequest(Data));
    }, 2000);
    ...
    <View style={styles.main}>
      ...
      <View style={{flexDirection: 'row'}}>
        ...
        <View style={[styles.codeTextInput, {}]}>
          <TextInput
            name="phoneNumber"
            defaultValue={saleData?.phoneNumber}
            control={control}
            rules={{
              required: {
                value: true,
...
export default EditSaleProperty2;
